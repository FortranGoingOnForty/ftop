...
void ftop_test_amdgpu_sysfs_cleanup(void) {
  char path[PATH_MAX];

  if (fixture_root[0] == '\0') return;
  if (join_path(path, sizeof(path), fixture_root, "card0/device/hwmon/hwmon7/temp1_input") == 0) (void)unlink(path);
  if (join_path(path, sizeof(path), fixture_root, "card0/device/hwmon/hwmon7/power1_average") == 0) (void)unlink(path);
  if (join_path(path, sizeof(path), fixture_root, "card0/device/hwmon/hwmon7/power1_cap") == 0) (void)unlink(path);
  if (join_path(path, sizeof(path), fixture_root, "card0/device/hwmon/hwmon7/pwm1") == 0) (void)unlink(path);
  if (join_path(path, sizeof(path), fixture_root, "card0/device/hwmon/hwmon7/pwm1_max") == 0) (void)unlink(path);
  if (join_path(path, sizeof(path), fixture_root, "card0/device/hwmon/hwmon7/name") == 0) (void)unlink(path);
  if (join_path(path, sizeof(path), fixture_root, "card0/device/hwmon/hwmon7") == 0) (void)rmdir(path);
  ...
  if (join_path(path, sizeof(path), fixture_root, "card0/device/pp_dpm_sclk") == 0) (void)unlink(path);
  if (join_path(path, sizeof(path), fixture_root, "card0/device/pp_dpm_mclk") == 0) (void)unlink(path);
...
static int create_card0(const char *root, int *sys_errno) {
  ...
  if (make_child_dir(sensor, sizeof(sensor), hwmon, "hwmon7", sys_errno) != 0) return -1;

  if (write_child_file(device, "vendor", "0x1002\n", sys_errno) != 0) return -1;
  if (write_child_file(device, "device", "0x744c\n", sys_errno) != 0) return -1;
  if (write_child_file(device, "product_name", "Fixture Radeon\n", sys_errno) != 0) return -1;
  if (write_child_file(device, "uevent", "DRIVER=amdgpu\nPCI_SLOT_NAME=0000:65:00.0\n", sys_errno) != 0) return -1;
  if (write_child_file(device, "gpu_busy_percent", "67\n", sys_errno) != 0) return -1;
  if (write_child_file(device, "mem_info_vram_total", "17179869184\n", sys_errno) != 0) return -1;
  if (write_child_file(device, "mem_info_vram_used", "8589934592\n", sys_errno) != 0) return -1;
  if (write_child_file(device, "pp_dpm_sclk", "0: 500Mhz\n1: 2500Mhz *\n", sys_errno) != 0) return -1;
  if (write_child_file(device, "pp_dpm_mclk", "0: 96Mhz\n1: 1200Mhz *\n", sys_errno) != 0) return -1;
  if (write_child_file(sensor, "name", "amdgpu\n", sys_errno) != 0) return -1;
  if (write_child_file(sensor, "temp1_input", "65000\n", sys_errno) != 0) return -1;
  if (write_child_file(sensor, "power1_average", "123000000\n", sys_errno) != 0) return -1;
  if (write_child_file(sensor, "power1_cap", "250000000\n", sys_errno) != 0) return -1;
  if (write_child_file(sensor, "pwm1", "128\n", sys_errno) != 0) return -1;
  return write_child_file(sensor, "pwm1_max", "255\n", sys_errno);
}
...
static int create_card1(const char *root, int *sys_errno) {
  ...
  if (make_child_dir(device, sizeof(device), card, "device", sys_errno) != 0) return -1;
  return write_child_file(device, "vendor", "0x8086\n", sys_errno);
}

int ftop_test_amdgpu_sysfs_create(char *path, size_t path_len, size_t *value_len, int *sys_errno) {
  ...
  *value_len = 0U;
  *sys_errno = 0;
  ftop_test_amdgpu_sysfs_cleanup();

  if (snprintf(template_path, sizeof(template_path), "/tmp/ftop-amdgpu.XXXXXX") >= (int)sizeof(template_path)) {
